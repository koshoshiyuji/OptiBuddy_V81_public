...
# 優先得意先オーダーの未完了重み
# ヒアリングセクション5「特定の得意先向けのオーダーは、優先的に期限内に完了させたい」に対応。
# 第1目的（未完了オーダー数最小化）において、優先得意先のオーダーが未完了の場合は
# 通常オーダーの PRIORITY_CUSTOMER_WEIGHT 倍のコストとして計上する。
# これにより、同じ完了件数でも優先得意先オーダーを先に完了させる方向にソルバーが誘導される。
PRIORITY_CUSTOMER_WEIGHT = 2  # 優先得意先の未完了1件 = 通常オーダー2件分のペナルティ
...
class EnergyCostAwareSchedulerSolver:
    ...
    def solve(self) -> dict:
        ...
        try:
            ...
            if engine == CPSAT:
                schedule, line_ops, feasible, metrics = self._solve_with_cpsat(
    ...
    def _build_and_solve(
        ...
        # 第1目的: 未完了オーダー数の最小化（優先得意先は重み付き）
        #   = Σ weight_i * (1 - presence_of(割当)) の最小化
        #   CP Optimizer は最小化のみのため、未割り当て件数（重み付き合計）を最小化する。
        #
        #   ヒアリングセクション5「特定の得意先向けのオーダーは、優先的に期限内に完了させたい」
        #   に対応するため、config["priority_customer_names"] に含まれる得意先のオーダーは
        #   PRIORITY_CUSTOMER_WEIGHT 倍の重みで未完了コストを計上する。
        #   これにより、優先得意先オーダーを完了させることが通常オーダーより強く優先される。
        ...
        for o in orders:
            ...
            weight = PRIORITY_CUSTOMER_WEIGHT if customer in priority_customers else 1
            itvs_for_order = list(order_itvs[oid].values())
            if itvs_for_order:
                assigned = mdl.sum([mdl.presence_of(itv) for itv in itvs_for_order])
                unfinished_terms.append(weight * (1 - assigned))
            else:
                unfinished_terms.append(weight)  # 対応ラインなし = 必ず未完了
    ...
    def _solve_with_cpsat(
        ...
        optional interval(variable-start, fixed-size)はstart/end/presentの3変数で表現し、
        pulse/cumulative制約はデマンド側に presence を掛けることで「不在なら消費0」を実現する
        （固定サイズが常に確保されても、demand=0であれば容量制約に影響しないため）。
        line_on_itvsは可変長(size不定)のoptional intervalなので、end>=startを明示制約する。
        目的関数はlexicographic(未完了最小化→総コスト最小化)。
        """
        import cpmpy as cp
        ...
        # order_vars[oid][lid] = (start, end, present)  (size=durは固定)
        order_vars: Dict[str, Dict[str, Tuple[Any, Any, Any]]] = {}
        for o in orders:
            oid      = str(o["id"])
            dur      = int(o.get("duration_min", 60))
            earliest = int(o.get("earliest_start_min", 0))
            ...
            for lid in compatible_lines:
                lo = earliest
                hi = max(earliest, end_latest - dur)
                start = cp.intvar(lo, hi, name=f"o_{oid}_l_{lid}_start")
                end = start + dur
                present = cp.boolvar(name=f"o_{oid}_l_{lid}_present")
                order_vars[oid][lid] = (start, end, present)

        # line_vars[lid] = (start, end, present)  (size不定=可変長)
        line_vars: Dict[str, Tuple[Any, Any, Any]] = {}
        for ln in lines:
            lid = str(ln["id"])
            start = cp.intvar(0, horizon, name=f"line_{lid}_start")
            end = cp.intvar(0, horizon, name=f"line_{lid}_end")
            present = cp.boolvar(name=f"line_{lid}_present")
            m += (end >= start)
            line_vars[lid] = (start, end, present)

        # 1. 各オーダーはどれか1つのラインに割り当てるか未割り当て
        for o in orders:
            oid = str(o["id"])
            presents_for_order = [v[2] for v in order_vars[oid].values()]
            if presents_for_order:
                m += (cp.sum(presents_for_order) <= 1)

        # 2. 各ラインのリソース上限(cumulative、demandにpresenceを掛けて不在時0にする)
        POWER_SCALE = 10
        ...
        for ln in lines:
            ...
            power_starts, power_durs, power_ends, power_demands = [], [], [], []
            worker_starts, worker_durs, worker_ends, worker_demands = [], [], [], []
            equip_starts, equip_durs, equip_ends, equip_demands = [], [], [], []

            for o in orders:
                oid = str(o["id"])
                if lid not in order_vars[oid]:
                    continue
                start, end, present = order_vars[oid][lid]
                dur = int(o.get("duration_min", 60))
                req_power   = float(o.get("power_kw", 0.0))
                req_workers = int(o.get("workers", 1))
                req_equip   = int(o.get("equipment_slots", 1))
                ...
                if req_power > 0:
                    power_starts.append(start); power_durs.append(dur); power_ends.append(end)
                    power_demands.append(present * int(round(req_power * POWER_SCALE)))
                if req_workers > 0:
                    worker_starts.append(start); worker_durs.append(dur); worker_ends.append(end)
                    worker_demands.append(present * req_workers)
                if req_equip > 0:
                    equip_starts.append(start); equip_durs.append(dur); equip_ends.append(end)
                    equip_demands.append(present * req_equip)
            ...
            if worker_starts:
                m += Cumulative(worker_starts, worker_durs, worker_ends, worker_demands, max_workers)
            if equip_starts:
                m += Cumulative(equip_starts, equip_durs, equip_ends, equip_demands, max_equipment)
        ...
        for ln in lines:
            ...
            for o in orders:
                ...
                if lid not in order_vars[oid]:
                    continue
                o_start, o_end, o_present = order_vars[oid][lid]
                m += o_present.implies(line_present)
                m += o_present.implies(o_start >= line_start)
                m += o_present.implies(o_end <= line_end)

        # 4. 境界条件: 計画期間の開始・終了でラインは停止
        for ln in lines:
            lid = str(ln["id"])
            line_start, line_end, line_present = line_vars[lid]
            m += line_present.implies(line_start >= 1)
            m += line_present.implies(line_end <= horizon - 1)
        ...
        for o in orders:
            ...
            presents_for_order = [v[2] for v in order_vars[oid].values()]
            if presents_for_order:
                assigned = cp.sum(presents_for_order)
                unfinished_terms.append(weight * (1 - assigned))
            else:
                unfinished_terms.append(weight)

        obj_unfinished = cp.sum(unfinished_terms) if unfinished_terms else cp.intvar(0, 0)
        ...
        for ln in lines:
            ...
            fixed_op_cost = startup_cost + shutdown_cost
            if fixed_op_cost > 0:
                cost_terms.append(line_present * int(fixed_op_cost * 100))
        ...
        for o in orders:
            ...
            if req_power == 0:
                continue
            for lid, (start, end, present) in order_vars[oid].items():
                avg_price = _average_tariff(tariff_table, slot_min)
                energy_cost = avg_price * req_power * (dur / 60.0)
                cost_terms.append(present * int(energy_cost * 100))
        ...
        for ln in lines:
            ...
            avg_price = _average_tariff(tariff_table, slot_min)
            standby_cost_per_min = avg_price * standby_power / 60.0
            cost_terms.append(line_present * (line_end - line_start) * int(standby_cost_per_min * 100))

        obj_cost = cp.sum(cost_terms) if cost_terms else cp.intvar(0, 0)

        solved, achieved = solve_lexicographic(
            m, [obj_unfinished, obj_cost],
        ...
        for o in orders:
            ...
            for lid, (start, end, present) in order_vars[oid].items():
                if not present.value():
                    continue
                start_min = int(start.value())
                end_min   = int(end.value())
                schedule.append({
                    "order_id":    oid,
                    "line_id":     lid,
                    "start":       start_min,
        ...
        for ln in lines:
            lid = str(ln["id"])
            line_start, line_end, line_present = line_vars[lid]
            if not line_present.value():
                continue
            start_min = int(line_start.value())
            end_min   = int(line_end.value())
            line_ops.append({
                "line_id":       lid,
                "line_name":     ln.get("name", lid),
                "start":         start_min,
        ...
        metrics = _compute_metrics(
            ...
            slot_min=slot_min,
            assigned_order_ids=assigned_order_ids,
            solve_time=meta.get("solve_time_sec") or 0.0,
