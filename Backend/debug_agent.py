...
def run_debug_agent(
    ...
    # 2026-09-07再改訂（Koshoshi合意）: 当初の文言は「未完了・未確認」とだけ
    # 述べており、「まだ確認できていないだけで、実際に問題があるとは限らない」
    # という区別が伝わらなかった（実機で、確認の結果ファイル変更なしで
    # 『問題なし』に終わったケースがあったにもかかわらず、この文言だけを見た
    # 段階では『修正が必要』というトーンに読めてしまっていた）。ここでは
    # 「最終確認が完了していないだけ」という位置づけを明示し、確認の結果
    # 何も直すべき点が見つからない場合もあることを推奨文に書き添える。
    max_turns_notes = [
        f"AIエージェントが最大ターン数（{max_turns}）に達したため、この点については"
        "最終確認（動作確認）が完了していません。実際に問題があるかどうかは、まだ"
        "分かっていません。"
    ]
    if edit_without_verify:
        max_turns_notes.append(
            "⚠️ 直前に加えた修正についても、最終確認（Gate2再検証）が実行されない"
            "まま終了しました。その修正が実際に効いているかどうかは未確認です。"
        )

    max_turns_notes.append(
        "🛠「修正を依頼する」から確認を続けるか、対象のシナリオを実際に解いて"
        "確認することをお勧めします（結果として直すべき点が見つからない場合も"
        "あります）。"
    )
