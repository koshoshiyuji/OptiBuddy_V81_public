...
## 依頼・案件間の移動時間の実装ルール（該当する場合は必ず守ること）

ヒアリングシート§9「依頼・案件間の移動時間に関するルール」でa（案件間の移動時間を
反映したい）が選択されている場合、拠点の有無によらず、同一資源（車両・作業者等）に
割り当てられる複数の案件が異なる地点（origin/destinationが一致しない）を持つときは、
transition_matrixを伴うno_overlap（本ファイル冒頭のTruckDispatcherパターンと同じ構造）を
用いて、依頼間の移動時間を明示的に制約に組み込むこと。「同一資源への複数案件の割当を
認める（相乗り等）が地点間の移動時間は考慮しない」実装は不可
（2026-09-21 MyPatientTransportPlannerで、同一車両が同時刻に別々の地点に存在する解が
生成される実害を確認済み）。

bが選択された場合、地点情報（origin/destination等）はスキーマ上保持するだけでよく、
移動時間制約への反映は不要。
...
### 依頼・案件間（拠点を除く）の移動時間に関する扱い
{structural_requirements.get('interleg_travel_notes') or '（記載なし・地点は全て同一 or 該当概念なし）'}
