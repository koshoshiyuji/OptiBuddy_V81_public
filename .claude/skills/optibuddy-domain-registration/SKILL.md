...
OptiBuddy V81（CPLEX CP Optimizerベースの最適化ドメイン自動生成プラットフォーム）で、ヒアリングシートから新規業務ドメインを登録する、または既存ドメインを再登録・修正するときの標準手順。CP/OR最適化とGate2検証ロジックの専門知識を前提に、Claude in Chromeでのボタンクリック連打ではなくFlask APIを直接叩くことで高速化する。初回のジョブ起動承認さえ得られれば、以降の指摘対応・登録可否の判断は上限付きでLLMが自律的に行う。ユーザーへの報告は「ユーザー向け進捗報告のルール」節に従い、進捗の区切りが分かる形で報告する（**指摘事項は診断結果に基づき自分の言葉で要約・推奨アクションを書く。バックエンドの指摘文言を未加工のまま転記しない**）。
...
**大原則（2026-09-07再改訂・2026-09-05版の方針を撤回）**: 2026-09-05版では
「バックエンド側のhumanize処理が既に平易な日本語に変換済みなので、スキルは
その文言をそのまま見せればよい」としていたが、これは誤りだった。実機テストで、
Gate2の動的検証由来の指摘（`run_gate2_dynamic_verification`が出すメッセージ、
例:「（実際に解いてみた結果が想定と違いました）[Gate2 lexicographic機構チェック]
WARNING: 参考: big_m=3 でも...」）はそもそも一度もhumanize処理を通っておらず、
内部の診断ログがそのままユーザーに出ていたことが判明した（2026-09-07）。
また、humanize処理自体もfast_model()（Haiku等）による、指摘文の文字列だけを
渡された狭い文脈での機械的な言い換えであり、実際のコード・ヒアリング内容を
見ずに書いているため、事実を薄めてしまう事故（2026-08-03、dynamic_exception系の
指摘で「実装のバグ」が「制約が満たせない」に変質した）も過去に発生している。
バックエンド側の言い換えが常に信頼できる前提には立てない。

**新しい方針**: 手順5・6で指摘事項を診断する際、対象の指摘に対応する
`job.questions`・`job.pending.advisory_questions`の原文（バックエンドの
言い換え結果を含む）は、あくまで診断のための入力データとして扱い、
**その文言をそのままユーザーに転記しない。** 実際にコード・シナリオ・
ヒアリング内容を確認して診断した結果に基づき、要約と推奨アクションを
自分の言葉でゼロから書く。ヒアリングの節番号・具体的な数値・項目名など
「これが何の話か」を照合するための手がかりは残すが、`Gate2`・`WARNING`・
`big_m`・`lexicographic`・`interval_var`・ファイルパス・変数名・関数名・
job_id等の内部用語は一切出さない。この方針により、Web画面（登録方法1、
バックエンドの文言をそのまま表示する）とスキル実行時（登録方法2、Claudeが
診断結果を書く）とで表示文言が異なることは許容する（もはや「入出力の内容は
同じであるべき」という前提には立たない。UIごとに適した提示方法でよい）。

**2026-09-07追加（Koshoshi合意・問題A対応）**: 上記の要約・推奨アクションは、
**技術的な選択（force_apply／続行／取りやめのどれを選ぶか）そのものをユーザーに
委ねるためのものではない。** この技術的判断は必ずClaude自身がCP/OR専門家として
下し、ユーザーには「その判断で進めてよいか」という承認のみを、はい/いいえで
答えられる形で求める（判断の中身自体を「どうしますか」とユーザーに開いたまま
投げない）。一方、ヒアリングシートに記載のない業務ルールの解釈など、業務専門家
であるユーザー本人にしか決められない事実確認が必要な場合は、Claudeが妥当と
考える具体的な回答案を1つ提示し、「この理解でよければ『はい』とだけ答えれば
よい」という形に必ず落とし込む。判断材料を何も示さないまま「どうしますか」
とだけ聞く、内部をよく知る人間しか答えられない開いた質問を投げることを禁止
する。

登録処理の進み具合をユーザーに伝えるときは、必ず下記の固定9ステップに沿って
「[ステップ X/9] ステップ名」を先頭に付けて報告する。API名・ファイルパス・
関数名・ハッシュ値・スタックトレース・HTTPステータス・job_id・
interval_var/presence_of等のCP/OR専門用語といった内部情報は出さない。
...
5. プログラムの自動作成
6. 作成内容のチェック
7. 確認事項への対応（診断結果に基づき自分で要約・推奨アクションを書いて提示する）
8. 登録の確定
...
- 【要回答】... ここで止まっている。ユーザーの回答・許可がないと先に進めない。
  返答してほしい形（例:「はい/いいえでお答えください」）も併記する。

技術的な詳細（コードの中身、API呼び出し、エラーメッセージの原文等）は、
ユーザーが「詳しく」「技術的な原因は」等と明示的に求めたときだけ出す。

**2026-09-06追加・2026-09-07更新: 自分の作業過程そのものを報告に含めない。** 診断のために取得した
生JSON・ファイルパス・変数名・ハッシュ値・job_id・backend.logの内容・ツールの
エラーメッセージ（タイムアウト等）に加え、`job.questions`等に入っているバックエンドの
指摘文言そのもの（`Gate2`・`WARNING`・`big_m`等の内部用語を含む場合がある）も、
あくまで自分の診断用の入力データであり、ユーザーへの報告にそのまま出さない。
ユーザーに見せるのは診断の結論と、それを支える平易な理由づけのみ。結論は必ず
先頭の1文で述べ、補足説明はその後に置く。
また、ユーザーが実行できない依頼（「コードを開いて確認してください」等）を
結びに付け足さない。判断に疑問が残る場合でも、ユーザーが実際に答えられる形
（例:「この理解で合っていますか？」）に留める。

**運用環境による差はつけない**: あなた自身が作業対象プロジェクトルートを
OptiBuddy_V81にして本スキルを使う場合も、上記の報告ルールは同様に適用する
（わかりやすさは環境を問わず追求する）。デバッグ目的の追加情報を付け足すこと自体は
妨げないが、それによってこの節が定める分かりやすい報告内容が埋もれたり
読みにくくなったりしないよう注意する。
...
- **Flaskはローカルの実プロセスであり、Claude側のBashサンドボックスから直接 `curl localhost:5000` 等ではアクセスできない**（別ネットワーク名前空間。`device_bash`経由でLAN IPを指定しても、通信許可リストにより届かないことを2026-09-06に確認済み）。API呼び出しは必ずClaude in Chromeの `javascript_tool`（`action: "javascript_exec"`）で、実際に開いているブラウザタブの中から `fetch()` を実行して行うこと。**タブは画面を人間に見せるためのものではなく、`fetch()`を実行するための実行環境（ネットワーク経路）に過ぎない**。取得した結果はその都度テキストとして報告すればよく、タブの見た目を確認する必要はない。
- **`javascript_exec`が45秒でタイムアウトした場合（タブが応答しなくなった状態）**: 同じタブへの同じ呼び出しを繰り返しリトライしない。直ちにそのタブを閉じ、新しいタブを作成し、Frontendの既知のURL（例: `http://localhost:5173/`）に`navigate`した上で、同じ処理を1回だけ再実行する。それでも失敗する場合は、それ以上リトライせずユーザーに事実を報告し指示を仰ぐ。
...
- **2026-09-06訂正**: 以前は「初回起動の承認さえ得れば、以降の`/api/domain/confirm`・`/api/domain/cancel`の呼び出しはLLMが自律的に行ってよい」としていたが、実際には環境側の権限設定により、これらのエンドポイントへの呼び出しは都度ブロックされ、ユーザーの明示的な承認が無いと実行できないことが判明した（2026-09-06実測）。**指摘内容の診断・技術的な分析自体は自律的に行ってよいが、`/api/domain/confirm`・`/api/domain/cancel`の実際の呼び出し（登録の確定・取りやめ）は、診断結果とAIの推奨を添えて必ず【要回答】としてユーザーに確認を求めてから実行する。** 自律実行を試みて権限エラーになってから報告するのではなく、最初から【要回答】として聞く（無駄な呼び出しを避けるため）。
- **自律実行の上限（無限ループ防止）**: 「続行（AIエージェントが対応）」も`/api/domain/confirm`（force_apply: false）を経由するため、上記の通り毎回ユーザーの【要回答】承認が必要。承認を得て実行した「続行」が同一ジョブに対して2回目のneeds_confirmationでも同種の指摘を解消できていない場合、3回目は自律的な「続行」提案をせず、その時点までの診断結果・試行履歴（何を指摘され、何を試し、なぜ解決しなかったか）を整理してユーザーに提示し、次のアクションの指示を仰ぐ。カウントはジョブ単位でリセットする。
...
このジョブ起動の承認を得た時点で、以降の指摘対応・診断（手順5〜7、上限あり）は自律的に進めてよい。ただし`/api/domain/confirm`・`/api/domain/cancel`の実行自体は、都度ユーザーの明示的な承認（【要回答】）を得てから行う（詳細は「絶対に守ること」節参照）。

### 4. 進捗ポーリング

**2026-09-05改訂・2026-09-07修正**: 以前は「fetchで確認→結果を見る→少し待つ→また確認」をLLMが逐次繰り返していたが、この「待つ／もう一度確認する」という判断そのものに毎回LLMの生成時間（数秒）がかかり、待機ステージが長いジョブほど無駄な時間が線形に積み重なっていた（実測: 8往復のポーリングだけで数十秒〜1分規模）。そのため2026-09-05に「ポーリングループを1回の`javascript_exec`呼び出しの中でブラウザ側に完結させる」方式に変更したが、**`javascript_exec`ツール自体に45秒の実行上限（Runtime.evaluate timeout）があることを2026-09-07に実機で確認した**。Stage2のコード生成のように1〜2分かかるステージがあると、ループ内部の安全上限（当時300000ms=5分）に達する前にツール自体が45秒でタイムアウトし、`javascript_exec`が失敗として返ってくる（内部で待っているJS自体は正常に動いており、ブラウザやFlaskが固まっているわけではない）。

これを踏まえ、**ループの内部上限をツールの実行上限より十分短く（35秒）取り、上限に達したら一旦その時点のstageを返して抜ける**方式にする。ジョブが完了していなければ、同じJSをそのまま再度呼び直す（間に追加の待機判断は不要、そのまま繰り返し呼ぶだけでよい）。1回あたり最大35秒×十数回分のfetch/待機をブラウザ側でまとめて行うため、2026-09-05改訂の「LLM側の待機判断コスト削減」という利点はほぼそのまま維持できる。

```js
let job;
const t0 = performance.now();
const INTERNAL_CAP_MS = 35000; // javascript_execツール自体の45秒上限より十分短くする
while (true) {
  ...
  if (performance.now() - t0 > INTERNAL_CAP_MS) break; // ここで一旦返す。まだ完了していなければ同じJSを再度呼ぶ
  await new Promise(res => setTimeout(res, 2000));
}
JSON.stringify({stage: job.stage, timeline: job.stage_timeline})
```

戻り値の`stage`が`needs_confirmation`・`done`・`error`のいずれでもなければ、同じJSをそのまま再度呼び直す（Stage2のコード生成は1〜2分程度かかることが多いため、2〜4回程度の再呼び出しが必要になる想定）。`job.stage_timeline` に各ステージの開始タイムスタンプ（UNIX epoch秒）が入っているので、後で処理時間の内訳を出せる。ユーザーへの進捗報告（「[ステップ4/9]...」等）は、最終的にジョブが完了状態になった後に1回まとめて行う（内部ループの再呼び出しのたびに報告メッセージを出す必要はない）。

**万一`javascript_exec`自体がタイムアウトエラーを返した場合**（35秒の内部上限より前に発生した場合、ネットワーク遅延等の異常事態）は、前提節に記載の通りタブを閉じて再作成し、同じJSで状態確認をやり直す。

### 5. 指摘が出たら盲目的に転記せず自分で診断する

`needs_confirmation` の場合、確認が必要な指摘は**トップレベルの`job.questions`**（Stage1aの`missing_info`とGate2の`blocking_questions`を結合したもの）に入っている。**`job.pending`の中には`blocking_questions`という独立したフィールドは存在しない。** `job.pending.advisory_questions`は、誤検知が多いと判明済みの参考情報カテゴリで、確認画面には出さない設計（advisoryしか残っていなければ人間の確認なしで進めてよいという既存方針）だが、記録のためpendingに保持されている。取得漏れを防ぐには、`job.questions`（確認必須）と`job.pending.advisory_questions`（記録用・参考情報として一応目を通す）を両方確認する。

**重大インシデント1（2026-09-05発生・診断ミスの直接原因）**: 以前のこの節の一括取得コードは`advisory: job.pending.advisory_questions || job.pending.questions`という書き方をしていた。`advisory_questions`フィールドが常に存在するため`||`の左側が常に真になり、blocking側の指摘が一度も取得・診断されないまま登録を確定してしまった。この状態で実装バグを見落とし、正常ケースが解けないドメインをforce_apply=trueで登録する事故が起きた。

**重大インシデント2（2026-09-06発生・上記の修正自体が誤っていたことが判明）**: インシデント1の修正として書かれた`blocking: job.pending.blocking_questions || []`は、**そもそもAPIに存在しないフィールドを参照していた**。JavaScriptは存在しないプロパティへのアクセスをエラーにせず黙って`undefined`を返すため、この種のバグは実行時に何のエラーも出さず、常に空配列を返し続ける。この状態で再度、本物のGate2 blocking_questions（`job.questions`の3件目、ヒアリング9節の`required_gap`指摘）を一度も見ないまま登録を確定する、同種の事故が再発した。**この教訓から、新しいAPIフィールドを参照するコードを書く前は、必ず`Object.keys(job)`・`Object.keys(job.pending)`で実際のキー一覧を確認してから書くこと（記憶や過去の文書だけでフィールド名を決めない）。**

```js
JSON.stringify({
  blocking: job.questions || [],  // missing_info + Gate2のblocking_questions（トップレベル。job.pendingの中には無い）
  advisory: job.pending.advisory_questions || [],  // 確認不要・記録用（誤検知が多いと判明済み）
  diffs: Object.values(job.pending.diffs || {}).map(d => ({path: d.path, content: d.new_content}))
})
```
...
**重大な仕様（2026-09-07判明。実機テストで初めて発覚した既存の仕組み。今回のセッションでの変更ではない）**: 上記3区分のうち「（プログラムのエラーで停止・要修正）」「（実際に解いてみた結果が想定と違いました）」の2つ（Backend/app.pyの`_DYNAMIC_STRUCTURAL_PREFIXES`）は、通常の`force_apply: true`単体では**バックエンド側が機械的に拒否する**（`force_apply_blocked: true`が返り、needs_confirmationのまま据え置かれる）。これは「Gate2動的検証は非交渉」という設計原則によるもので、事前solve検証で問題が再現しなかった（＝診断の結果、実装バグではないと確認できた）場合であっても、`force_apply: true`だけでは通らない。この場合は下記「診断済み・実装バグではないと確認できた場合」の`dynamic_override`を使うこと。「（特殊な条件の扱いに矛盾の疑い）」はこのブロック対象に含まれず、通常のforce_apply: trueで問題なく通る。

**いずれの選択肢も、実行前に診断結果と推奨アクションを添えて【要回答】としてユーザーに確認を取ってから`/api/domain/confirm`・`/api/domain/cancel`を呼ぶ（「絶対に守ること」節参照）。** 選択基準（手順5の診断結果に基づく）:
...
await fetch('http://localhost:5000/api/domain/confirm', {
  method: 'POST', headers: {'Content-Type': 'application/json'},
  body: JSON.stringify({domain_name: '<DomainName>', pending: job.pending, questions: job.questions, answers: '<診断理由>', job_id: '<job_id>', force_apply: true})
}).then(r=>r.json())
```
  （「指摘を残したまま登録する」相当）。判断理由を簡潔にユーザーへ報告する。
- 「（プログラムのエラーで停止・要修正）」「（実際に解いてみた結果が想定と違いました）」のいずれかに該当する指摘があり、かつ事前solve検証で問題が再現せず、実装バグではないと確認できた → 通常のforce_applyでは拒否されるため、`dynamic_override: true`と、実装バグではないと判断した理由（`answers`、空文字不可）を付けて叩く。
```js
await fetch('http://localhost:5000/api/domain/confirm', {
  method: 'POST', headers: {'Content-Type': 'application/json'},
  body: JSON.stringify({domain_name: '<DomainName>', pending: job.pending, questions: job.questions, answers: '<実装バグではないと判断した具体的な理由>', job_id: '<job_id>', force_apply: true, dynamic_override: true})
}).then(r=>r.json())
```
  この理由は監査ログ・完了画面の両方に残る（隠せない・隠さない）。事前solve検証をせずにこれを使うことは禁止する。
...
2026-09-08追記（Koshoshi合意・実機テスト登録で判明）: 上記のstage_timelineは`needs_confirmation`到達までの完全自動処理区間しかカバーしていない。実際には`needs_confirmation`到達後、Claudeが指摘内容を診断し報告文を作成してから実際に`/api/domain/confirm`または`/api/domain/cancel`を呼ぶまでにも相応の時間がかかっており（実測で自動処理区間の3倍以上になったケースあり）、これを内訳に含めずに「処理時間」と報告すると体感時間との乖離が大きくなる。そのため、`needs_confirmation`のタイムスタンプと実際に`/api/domain/confirm`/`cancel`を呼んだ時刻の差分も、AIによる自動処理時間とは区別した上で、当面は内訳に含めてユーザーに報告すること。
